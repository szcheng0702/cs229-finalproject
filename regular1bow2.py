...
if __name__ == "__main__":
    ...
    X_competition2, _ = generate_features_add_bow(competition_dataset.stances, competition_dataset, "competition")
    ...
    X_holdout2, _ = generate_features_add_bow(hold_out_stances, d, "holdout")

    for fold in fold_stances:
        Xs[fold], ys[fold] = generate_features(fold_stances[fold], d, str(fold))
        Xs2[fold], _ = generate_features_add_bow(fold_stances[fold], d, str(fold))
    ...
    for fold in fold_stances:
        ...
        X_train = np.vstack(tuple([Xs2[i] for i in ids]))
        ...
        X_test2 = Xs[fold]# using a different set of features
    ...
    h_results = best_model1.predict(X_holdout2)
    for index, item in enumerate(list(h_results)):
        if item != 3:
            h_results[index] = best_model2.predict(np.array(X_holdout1[index]).reshape(1, -1))
    ...
    t_results = best_model1.predict(X_competition2)
    for index, item in enumerate(list(t_results)):
        if item != 3:
            t_results[index] = best_model2.predict(np.array(X_competition1[index]).reshape(1, -1))
